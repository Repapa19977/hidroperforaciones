...
interface ResumenGastos {
  ...
  ejecutadoQ: number      // compras reales del Libro de Compras
  avancePct: number
  desviacionQ: number
}
...
export default function ControlGastosPage() {
  ...
  const [filtro, setFiltro] = useState<'todos' | 'activo' | 'pausado' | 'completado' | 'cancelado'>('activo')
    ...
    <div className="flex flex-col md:h-full bg-[#070d1a]">
      <div className="px-4 sm:px-6 pt-4 sm:pt-5 pb-3 sm:pb-4 border-b border-white/5 bg-[#0a1020] shrink-0">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          <KPICard icon={<Wallet className="w-4 h-4 text-blue-400" />}     label="Presupuesto total" value={formatQ(totalPresupuesto)} sub="aprobado"   color="blue" />
          <KPICard icon={<TrendingUp className="w-4 h-4 text-violet-400" />} label="Comprado real"        value={formatQ(totalEjecutado)}  sub={totalPresupuesto > 0 ? `${((totalEjecutado/totalPresupuesto)*100).toFixed(0)}% del total` : '—'} color="violet" />
          <KPICard icon={<AlertTriangle className="w-4 h-4 text-amber-400" />} label="En alerta"      value={String(enAlerta)}         sub="sobre presupuesto" color={enAlerta > 0 ? 'amber' : 'slate'} />
          <KPICard icon={<CheckCircle className="w-4 h-4 text-emerald-400" />} label="Al día"        value={String(filtrados.length - enAlerta)} sub="bajo presupuesto" color="emerald" />
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {(['todos', 'activo', 'pausado', 'completado', 'cancelado'] as const).map(s => (
            <button key={s} onClick={() => setFiltro(s)}
              ...
              {s === 'todos' ? 'Todos' : s === 'activo' ? 'Activos' : s === 'pausado' ? 'Pausados' : s === 'cancelado' ? 'Cancelados' : 'Completados'}
            </button>
          ))}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex-1 md:overflow-auto p-3 sm:p-4 space-y-3">
          {filtrados.map(p => {
            ...
            return (
              <Link key={p.id} href={`/gastos/${p.id}`}
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className={cn('text-[10px] px-1.5 py-0.5 rounded', estadoBadgeClass(p.estado))}>{p.estado}</span>
                      {sobreExtrap && (
                ...
                {r ? (
                  <div>
                    {/* Comprado real: unica metrica financiera en Control de Gastos.
                        El avance fisico vive en Bitacora (modulo independiente). */}
                    <div className="flex items-center justify-between text-[10px] mb-1">
                      <span className="text-slate-500">Comprado / Presupuesto</span>
